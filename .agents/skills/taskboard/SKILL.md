...
`--gate` 用于会阻塞大片后续工作的关键节点，`--accept` 写可检查的验收条件，
`--branch` 和 `--pr` 记录代码坐标。

## 写入或审查文案前读取统一标准

新增、编辑或审查需求简介、任务、finding、risk、link 时，必须先读
[看板文案标准](references/writing-standard.md)。该文件是字段标签、标题句式、状态边界、
分类和生命周期规则的唯一维护源；不要在本文件另写一套近似规则。
