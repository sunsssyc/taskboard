...
def prepare(output_dir: Path):
    return subprocess.run(
        [
            sys.executable, str(SCRIPT), '--repository', 'example/claude-taskboard',
...
def test_prepare_homebrew_release_is_deterministic_and_complete(tmp_path):
    ...
    archive_name = 'claude-taskboard-0.1.0.tar.gz'
    ...
    assert 'claude-taskboard-0.1.0/taskboard/cli.py' in names
    assert 'claude-taskboard-0.1.0/.agents/skills/taskboard/SKILL.md' in names

    formula = (first / 'taskboard.rb').read_text(encoding='utf-8')
    assert 'https://github.com/example/claude-taskboard/releases/download/v0.1.0/' in formula
